...
object SolicitudCustodiaJuzgadoPdfGenerator {
    ...
    fun generatePdf(
        context: Context,
        data: SolicitudCustodiaJuzgadoData,
        ...
        loadBitmapFromAssets(context, "images/EscEspana.png")?.let { bmp ->
            ...
            canvas.drawBitmap(bmp, null, RectF(leftShieldX, leftShieldY, leftShieldX + leftShieldW, leftShieldY + leftShieldH), null)
        }
        ...
        loadBitmapFromAssets(context, "images/EscGuardiaCivil.png")?.let { bmp ->
            ...
            canvas.drawBitmap(bmp, null, RectF(rightX - mmToPt(3f) - rightShieldW, rightShieldY, rightX - mmToPt(3f), rightShieldY + rightShieldH), null)
        }
        ...
        val asuntoText = "ASUNTO:   \"Solicitud de mandamiento judicial de requerimiento a un centro sanitario para la realización de analítica a muestras de sangre obtenidas con fines terapéuticos.\""
        ...
        // PÁRRAFO 1
        // ═══════════════════════════════════════════════════════════════════
        val splitParts = data.fechaHoraSiniestro.split(" ", limit = 2)
        val siniestroFecha = splitParts.getOrElse(0) { "" }
        val siniestroHora = splitParts.getOrElse(1) { "" }
        ...
        val paragraph1 = "Para el esclarecimiento del siniestro vial ocurrido a las ${siniestroHora.ifBlank { "___" }} horas del día ${sinDia.ifBlank { "___" }} de ${monthNumberToSpanish(sinMes)} del ${sinAnio.ifBlank { "____" }}," +
            " en ${data.carretera.ifBlank { "________" }} término municipal de ${data.municipioOcurrencia.ifBlank { "________" }} y Partido Judicial de ${data.partidoJudicial.ifBlank { "________" }}," +
            " con el resultado de ${data.catalogacionHecho.ifBlank { "________________________________" }} y por el que se instruye en esta Unidad" +
            " atestado núm. ${data.numeroDiligencia.ifBlank { "________" }} y toda vez que existen indicios racionales de que el conductor de un vehículo implicado" +
            " podia haber conducido bajo la influencia de alcohol, estupefacientes, psicotrópicos u otras sustancias estimulantes o" +
            " análogas, consistentes en:"
        ...
        // PÁRRAFO 2
        // ═══════════════════════════════════════════════════════════════════
        val paragraph2 = "En virtud de lo preceptuado en el artículo 796.1.7ª de la Ley de Enjuiciamiento Criminal, en relación con el artículo" +
            ...
            " V. I. se dicte oficio o mandamiento judicial de requerimiento al Centro Sanitario ${data.centroSanitario.ifBlank { "________" }}," +
            " para que realice analítica de las muestras de sangre extraídas con fines terapéuticos, al conductor implicado que posteriormente se" +
            " reseña, al objeto de la determinación de la tasa de alcohol en sangre, así como la presencia y cantidad de drogas tóxicas," +
            " estupefacientes y psicotrópicos presentes en su organismo, lo cual podría ser determinante como fuente de prueba para" +
            " demostrar una posible imprudencia grave constitutiva de delito y/o la comisión de un delito contra la seguridad vial:"
        y = drawWrappedText(paragraph2, contentLeft, contentRight, y)
        y += mmToPt(4f)

        // ═══════════════════════════════════════════════════════════════════
        // TABLA: NOMBRE Y APELLIDOS / D.N.I. / MATRÍCULA
        // ═══════════════════════════════════════════════════════════════════
        val tableLeft = contentLeft + mmToPt(5f)
        val tableRight = contentRight - mmToPt(5f)
        val tableWidth = tableRight - tableLeft
        val col1Width = tableWidth * 0.55f
        val col2Width = tableWidth * 0.22f
        ...
        canvas.drawLine(col2Right, tableTop, col2Right, tableTop + rowHeight, linePaint)
        ...
        val hdr3 = "MATRÍCULA"
        val hdr3W = tableBoldPaint.measureText(hdr3)
        canvas.drawText(hdr3, col2Right + (col3Width - hdr3W) / 2f, headerTextY, tableBoldPaint)
        ...
        canvas.drawLine(col2Right, dataRowTop, col2Right, dataRowTop + rowHeight, linePaint)
        ...
        canvas.drawText(data.matricula.ifBlank { "________" }, col2Right + mmToPt(2f), dataTextY, tableTextPaint)
        ...
        // PÁRRAFO 3
        // ═══════════════════════════════════════════════════════════════════
        val entregaSplit = data.fechaEntregaOficio.split("-")
        val entregaDia = entregaSplit.getOrElse(0) { "___" }
        val entregaMes = entregaSplit.getOrElse(1) { "___" }
        val entregaAnio = entregaSplit.getOrElse(2) { "____" }

        val paragraph3 = "Le significo, que con fecha ${entregaDia.ifBlank { "___" }} de ${monthNumberToSpanish(entregaMes)} de ${entregaAnio.ifBlank { "____" }}" +
            " por la Fuerza instructora se ha entregado oficio en el centro sanitario, para que se adopten las medidas oportunas en orden a la custodia y conservación de las muestras de sangre extraídas con fines terapéuticos," +
            " hasta que Su Señoría dicta la resolución que estime procedente, en relación a la presente solicitud."
        ...
        // PÁRRAFO 4 - CIERRE
        // ═══════════════════════════════════════════════════════════════════
        val paragraph4 = "Lo que participo a V.I., para su conocimiento y a los efectos pertinentes."
        ...
        val file = File(directory, "SolicitudCustodiaJuzgado.pdf")
