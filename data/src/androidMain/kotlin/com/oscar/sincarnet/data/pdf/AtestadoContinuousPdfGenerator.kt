...
import com.oscar.sincarnet.data.document.CitacionDocument
import com.oscar.sincarnet.data.document.CitacionItem
import com.oscar.sincarnet.data.document.CitacionOpcion
import com.oscar.sincarnet.data.document.CitacionSeccion
import com.oscar.sincarnet.data.document.loadCitacionDocument
import com.oscar.sincarnet.data.document.replaceCitacionPlaceholders
import com.oscar.sincarnet.data.document.replaceManifestacionPlaceholders
...
private val atestadoFilesBeforeCitation = listOf(
    "01inicio.json",
    "02derechos.json",
...
private val atestadoFilesAfterCitation = listOf(
    "05inmovilizacion.json"
)

private val completeAtestadoExtraFiles = listOf(
    "09denunciasformuladas.json",
    "10antecedentes.json",
    "11envio.json"
)
 ...
 */
fun generateAtestadoContinuousPdf(
    context: Context,
    ...
    fun appendTemplates(fileNames: List<String>) {
    ...
    appendTemplates(atestadoFilesBeforeCitation)
    ...
    appendTemplates(atestadoFilesAfterCitation)
    if (includeCompleteDiligencias) appendTemplates(completeAtestadoExtraFiles)
    ...
    orderedDocuments.forEachIndexed { documentIndex, document ->
        ...
        if (document.onlyEnterado) {
        ...
        } else if (document.onlyInvestigatedSignature) {
            drawSoloInvestigadoSignature(
                canvas = pageState.canvas,
                signatures = signatures,
                startY = pageState.cursorY + 6f,
            ...
            drawCitacionSignatures(
                ...
                tipPaint = textPaintSmall,
                hasSecondDriver = hasSecondDriver,
                includeInvestigated = !document.omitInvestigatedSignature,
...
private fun buildAtestadoOdtContentXml(
    ...
    orderedDocuments.forEachIndexed { index, document ->
        ...
        if (document.onlyEnterado || document.onlyInvestigatedSignature) {
            appendSignatureBlock(
                title = if (document.onlyEnterado) "Enterado" else "Investigado",
            ...
            if (!document.omitInvestigatedSignature) {
                appendSignatureBlock(
                    title = "Investigado",
                    asset = signatureAssets[PdfSignatureSlot.INVESTIGATED],
                    tip = null,
...
internal fun drawCitacionHeader(
...
private fun loadAtestadoTemplateAsCitacion(
    ...
    return runCatching {
        ...
        if (fileName == "09denunciasformuladas.json") {
            val denuncias = root.optJSONObject("denuncias_formuladas")
            return@runCatching CitacionDocument(
                titulo = title,
                cuerpoDescripcion = body,
                secciones = listOf(
                    CitacionSeccion(
                        titulo = "Denuncias formuladas",
                        contenido = denuncias?.optString("descripcion", "").orEmpty(),
                        items = listOf(
                            CitacionItem(
                                denuncias?.optJSONObject("plantilla_denuncia")
                                    ?.optString("texto", "")
                                    .orEmpty()
                            )
                        )
                ...
                cierre = root.optJSONObject("cierre")?.optString("descripcion", "").orEmpty(),
                omitInvestigatedSignature = true
            )
        }

        if (fileName == "10antecedentes.json") {
            val apartados = root.optJSONObject("antecedentes")?.optJSONArray("apartados")
            val sections = buildList {
                for (index in 0 until (apartados?.length() ?: 0)) {
                    val apartado = apartados?.optJSONObject(index) ?: continue
                    add(
                        CitacionSeccion(
                            titulo = apartado.optString("titulo", ""),
                            contenido = apartado.optString("campo_variable", "")
                        )
                    )
                }
            }
            ...
            return@runCatching CitacionDocument(
                ...
                secciones = listOf(
                    CitacionSeccion(
                        ...
                        contenido = root.optJSONObject("documentacion_adjunta")
                            ?.optString("descripcion", "")
                            .orEmpty(),
                        items = listOf(CitacionItem("Diligencias de documentos escaneados"))
                    ),
                    CitacionSeccion(
                        titulo = "Remisión al Ministerio Fiscal",
                        contenido = "Se significa que se remite al Ministerio Fiscal copia del presente Atestado."
                    )
                ),
                cierre = root.optJSONObject("cierre")?.optString("descripcion", "").orEmpty(),
                omitInvestigatedSignature = true
        ...
        val sectionKeys = when (fileName) {
            ...
            "13letradogratis.json" -> listOf("articulos")
            else -> emptyList()
        }
        ...
        CitacionDocument(
            ...
            ,
            onlyInvestigatedSignature = fileName == "13letradogratis.json"
            ,
            omitInvestigatedSignature = fileName == "09denunciasformuladas.json" ||
                fileName == "10antecedentes.json" ||
                fileName == "11envio.json"
            ,
