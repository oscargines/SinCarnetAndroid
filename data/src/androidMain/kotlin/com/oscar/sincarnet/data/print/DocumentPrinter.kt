...
import com.oscar.sincarnet.data.document.replaceCitacionPlaceholders
...
object DocumentPrinter {
     ...
     */
    const val MANIFESTACION_REQUIRED_MSG =
    ...
    /**
     * Traduce excepciones del SDK de Zebra y errores comunes de Bluetooth
    ...
    fun friendlyErrorMessage(e: Exception): String {
        ...
        return when {
            raw.contains("Could not connect", ignoreCase = true) ->
                "No se pudo conectar con la impresora. Verifique que esté encendida y emparejada."
            raw.contains("connect timed out", ignoreCase = true) ||
            raw.contains("connection timed out", ignoreCase = true) ->
                "La conexión con la impresora ha tardado demasiado. Intente de nuevo."
            raw.contains("read timed out", ignoreCase = true) ->
                "La impresora no responde. Compruebe que tiene papel y está cerca del dispositivo."
            raw.contains("BluetoothSocket", ignoreCase = true) ||
            raw.contains("unable to start", ignoreCase = true) ->
                "Error al establecer conexión Bluetooth. Active el Bluetooth y reintente."
            raw.contains("permission", ignoreCase = true) ||
            raw.contains("SecurityException", ignoreCase = true) ->
                "Permiso de Bluetooth denegado. Conceda los permisos en la configuración del dispositivo."
            raw.contains("broken pipe", ignoreCase = true) ||
            raw.contains("connection reset", ignoreCase = true) ->
                "La conexión con la impresora se ha interrumpido. Intente de nuevo."
            raw.contains("No such device", ignoreCase = true) ->
                "Impresora no encontrada. Verifique que la impresora esté emparejada."
            raw.isEmpty() ->
                "Error desconocido al imprimir."
            else ->
                "Error al imprimir: $raw"
        }
    }
    ...
    fun imprimirDerechos(context: Context, mac: String?) {
        val ocurrencia = OcurrenciaDelitStorage(context.toStorage("ocurrencia_delit_storage")).loadCurrent()
        val juzgado = JuzgadoAtestadoStorage(context.toStorage("juzgado_atestado_storage")).loadCurrent()
        val actuantes = ActuantesStorage(context.toStorage("actuantes_storage")).loadCurrent()
        val investigado = PersonaInvestigadaStorage(context.toStorage("persona_investigada_storage")).loadCurrent()
    ...
    fun imprimirManifestacion(context: Context, mac: String?) {
        ...
        val juzgado = JuzgadoAtestadoStorage(context.toStorage("juzgado_atestado_storage")).loadCurrent()
        val investigado = PersonaInvestigadaStorage(context.toStorage("persona_investigada_storage")).loadCurrent()
        val vehiculo = VehiculoStorage(context.toStorage("vehiculo_storage")).loadCurrent()
        val manifestacion = ManifestacionStorage(context.toStorage("manifestacion_storage")).loadCurrent()
    ...
    suspend fun imprimirInicioSuspend(
        ...
        val investigado = PersonaInvestigadaStorage(context.toStorage("persona_investigada_storage")).loadCurrent()
        val vehiculo = VehiculoStorage(context.toStorage("vehiculo_storage")).loadCurrent()
        val inicioData = AtestadoInicioStorage(context.toStorage("atestado_inicio_storage")).loadCurrent()
    ...
    // ============================================================
    // imprimirActaTrasladoSuspend
    // Imprime el acta de traslado de vehículo en impresora Zebra.
    // ============================================================
    suspend fun imprimirActaTrasladoSuspend(
        context: Context,
        mac: String,
        agenteSignature: ImageBitmap? = null,
        titularSignature: ImageBitmap? = null,
        sharedConn: Connection? = null
    ) {
        android.util.Log.d("Impresion", "[imprimirActaTrasladoSuspend] INICIO")
        val storage = ActaTrasladoVehiculoStorage(context.toStorage("acta_traslado_vehiculo_storage"))
        val data = storage.loadCurrent()

        val nombreCompleto = listOf(data.nombre, data.primerApellido, data.segundoApellido)
            .filter { it.isNotBlank() }.joinToString(" ")
        val tipoPersona = when {
            data.esTitular && data.esConductor -> "TITULAR/CONDUCTOR"
            data.esTitular -> "TITULAR"
            data.esConductor -> "CONDUCTOR"
            else -> ""
        }
        val checkAuxilioAccidente = if (data.inicioAuxilioAccidente) "X" else " "
        val checkInfraccion = if (data.inicioInfraccion) "X" else " "
        val checkOtroMotivo = if (data.inicioOtroMotivo.isNotBlank()) "X" else " "
        val checkFactoresAtmosfericos = if (data.circunstanciaFactoresAtmosfericos) "X" else " "
        val checkMalaVisibilidad = if (data.circunstanciaMalaVisibilidad) "X" else " "
        val checkConfiguracionVia = if (data.circunstanciaConfiguracionVia.isNotBlank()) "X" else " "
        val checkOtrasCircunstancias = if (data.circunstanciaOtras.isNotBlank()) "X" else " "
        val inicioFechaHora = "${data.inicioFecha} ${data.inicioHora}".trim()
        val finFechaHora = "${data.finFecha} ${data.finHora}".trim()
        val consentimientoTraslado = when (data.consentimientoTraslado) {
            true -> "Sí"
            false -> "No"
            else -> ""
        }
        val personaAutoriza = buildString {
            if (data.autorizaTitular) append("TITULAR")
            if (data.autorizaConductor) {
                if (isNotEmpty()) append(" / ")
                append("CONDUCTOR")
            }
        }
        val firmanteAutoriza = when (data.firmanteAutorizaTraslado) {
            true -> "Sí"
            false -> "No"
            else -> ""
        }
        val incidencia = when (data.incidenciaDuranteTraslado) {
            true -> "Sí"
            false -> "No"
            else -> ""
        }
        val entregaLlaves = when (data.entregaLlaves) {
            true -> "Sí"
            false -> "No"
            else -> ""
        }
        ...
        val placeholders = mutableMapOf(
            "sector" to (data.sector.ifBlank { "____________________" }),
            "subsector" to (data.subsector.ifBlank { "____________________" }),
            "destacamento" to (data.destacamento.ifBlank { "____________________" }),
            "vehiculoTipo" to data.vehiculoTipo,
            "vehiculoMarca" to data.vehiculoMarca,
            "vehiculoModelo" to data.vehiculoModelo,
            "vehiculoColor" to data.vehiculoColor,
            "vehiculoMatricula" to data.vehiculoMatricula,
            "tipoPersona" to tipoPersona,
            "nombrecompleto" to nombreCompleto,
            "dniNie" to data.dniNie,
            "telefono" to data.telefono,
            "checkAuxilioAccidente" to checkAuxilioAccidente,
            "checkInfraccion" to checkInfraccion,
            "checkOtroMotivo" to checkOtroMotivo,
            "inicioOtroMotivo" to data.inicioOtroMotivo,
            "checkFactoresAtmosfericos" to checkFactoresAtmosfericos,
            "checkMalaVisibilidad" to checkMalaVisibilidad,
            "checkConfiguracionVia" to checkConfiguracionVia,
            "circunstanciaConfiguracionVia" to data.circunstanciaConfiguracionVia,
            "checkOtrasCircunstancias" to checkOtrasCircunstancias,
            "circunstanciaOtras" to data.circunstanciaOtras,
            "inicioLugar" to data.inicioLugar,
            "inicioFechaHora" to inicioFechaHora,
            "finLugar" to data.finLugar,
            "finFechaHora" to finFechaHora,
            "unidadResponsable" to data.unidadResponsable,
            "unidadTelefono" to data.unidadTelefono,
            "agenteTip" to data.agenteTip,
            "agenteUnidad" to data.agenteUnidad,
            "consentimientoTraslado" to consentimientoTraslado,
            "personaAutoriza" to personaAutoriza,
            "firmanteAutorizaTraslado" to firmanteAutoriza,
            "incidenciaDuranteTraslado" to incidencia,
            "entregaLlaves" to entregaLlaves
        )

        val raw = context.assets.open("docs/07actatraslado.json").bufferedReader().readText()
        val json = JSONObject(raw)
        val doc = json.getJSONObject("documento")
        val titulo = resolve(doc.optString("titulo", ""), placeholders)
        val body = buildTrasladoBody(doc, placeholders)

        val sigs = PrintSignatures(
            agenteSignature = agenteSignature,
            titularSignature = titularSignature,
            isTraslado = true
        )

        BluetoothPrinterUtils.printTrasladoDocumentSuspend(
            context, mac, titulo, body, sigs,
            trasladoData = placeholders,
            sharedConn = sharedConn
        )
        android.util.Log.d("Impresion", "[imprimirActaTrasladoSuspend] finalizado")
    }

    // ============================================================
    // imprimirActaTrasladoCompleto
    // Abre UNA SOLA conexión BT → imprime el acta → cierra la conexión.
    // ============================================================
    fun imprimirActaTrasladoCompleto(
        context: Context,
        mac: String,
        agenteSignature: ImageBitmap? = null,
        titularSignature: ImageBitmap? = null,
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            try {
                android.util.Log.i(BATCH_LOG_TAG, "Inicio impresión acta traslado, mac=$mac")
                withContext(Dispatchers.Main) {
                    onProgress(0, 1, "Conectando con la impresora…")
                }
                sharedConn = openSharedBtConnection(mac)

                withContext(Dispatchers.Main) {
                    onProgress(1, 1, "Acta de traslado de vehículo")
                }
                imprimirActaTrasladoSuspend(context, mac, agenteSignature, titularSignature, sharedConn)

                android.util.Log.i(BATCH_LOG_TAG, "Acta de traslado impresa correctamente")
                withContext(Dispatchers.Main) { onFinished() }
            } catch (e: Exception) {
                android.util.Log.e(BATCH_LOG_TAG, "Error al imprimir acta traslado: ${e.message}", e)
                withContext(Dispatchers.Main) { onError(friendlyErrorMessage(e)) }
            } finally {
                sharedConn?.let {
                    android.util.Log.i(BATCH_LOG_TAG, "Espera final antes de cerrar BT: ${FINAL_DRAIN_BEFORE_CLOSE_MS} ms")
                    kotlinx.coroutines.delay(FINAL_DRAIN_BEFORE_CLOSE_MS)
                    closeSharedBtConnection(it)
                }
                android.util.Log.i(BATCH_LOG_TAG, "Conexión BT de acta traslado cerrada")
            }
        }
    }

    // ============================================================
    // imprimirActaTrasladoCentroSanitarioSuspend
    // Imprime el acta de traslado a centro sanitario en impresora Zebra.
    // ============================================================
    suspend fun imprimirActaTrasladoCentroSanitarioSuspend(
        context: Context,
        mac: String,
        ...
        android.util.Log.d("Impresion", "[imprimirActaTrasladoCentroSanitarioSuspend] INICIO")
        val storage = ActaTrasladoCentroSanitarioStorage(context.toStorage("centro_sanitario_storage"))
        val data = storage.loadCurrent()

        val checkPruebaSangre = if (data.pruebaSangre) "X" else " "
        val checkPruebaOtroTipo = if (data.pruebaOtroTipo) "X" else " "

        val placeholders = mutableMapOf(
            "subsector" to (data.subsector.ifBlank { "____________________" }),
            "unidadInterviniente" to (data.unidadInterviniente.ifBlank { "____________________" }),
            "numeroExpediente" to (data.numeroExpediente.ifBlank { "____________________" }),
            "lugarIntervencion" to data.lugarIntervencion,
            "motivo" to data.motivo,
            "fechaHora" to data.fechaHora,
            "nombreApellidos" to data.nombreApellidos,
            "dniNiePasaporte" to data.dniNiePasaporte,
            "facultativoColegiado" to data.facultativoColegiado,
            "checkPruebaSangre" to checkPruebaSangre,
            "checkPruebaOtroTipo" to checkPruebaOtroTipo,
            "pruebaOtroDetalle" to data.pruebaOtroDetalle,
            "centroSanitario" to data.centroSanitario,
            "extraccionColegiado" to data.extraccionColegiado,
            "fechaHoraExtraccion" to data.fechaHoraExtraccion,
            "precintosSeguridad" to data.precintosSeguridad,
            "datoMuestra" to data.datoMuestra,
            "juzgadoOrganismo" to data.juzgadoOrganismo,
            "otrasObservaciones" to data.otrasObservaciones,
            "numTip" to data.numTip
        )

        val raw = context.assets.open("docs/08actatrasladocsanitario.json").bufferedReader().readText()
        val json = JSONObject(raw)
        val doc = json.getJSONObject("documento")
        val titulo = resolve(doc.optString("titulo", ""), placeholders)
        val body = buildCentroSanitarioBody(doc, placeholders)

        val sigs = PrintSignatures(
            interesado = interesadoSignature,
            facultativo = facultativoSignature,
            sanitario = sanitarioSignature,
            agente = agenteSignature,
            isCentroSanitario = true
        )

        BluetoothPrinterUtils.printTrasladoCentroSanitarioDocumentSuspend(
            context, mac, titulo, body, sigs, sharedConn
        )
        android.util.Log.d("Impresion", "[imprimirActaTrasladoCentroSanitarioSuspend] finalizado")
    }

    // ============================================================
    // imprimirActaTrasladoCentroSanitarioCompleto
    // Abre UNA SOLA conexión BT → imprime el acta → cierra la conexión.
    // ============================================================
    fun imprimirActaTrasladoCentroSanitarioCompleto(
        context: Context,
        mac: String,
        interesadoSignature: ImageBitmap? = null,
        facultativoSignature: ImageBitmap? = null,
        ...
        CoroutineScope(Dispatchers.IO).launch {
            var sharedConn: Connection? = null
            try {
                android.util.Log.i(BATCH_LOG_TAG, "Inicio impresión acta traslado centro sanitario, mac=$mac")
                withContext(Dispatchers.Main) {
                    onProgress(0, 1, "Conectando con la impresora…")
                }
                sharedConn = openSharedBtConnection(mac)
                ...
                withContext(Dispatchers.Main) {
                    onProgress(1, 1, "Acta de traslado a centro sanitario")
                }
                imprimirActaTrasladoCentroSanitarioSuspend(
                    context, mac, interesadoSignature, facultativoSignature,
                    sanitarioSignature, agenteSignature, sharedConn
                )

                android.util.Log.i(BATCH_LOG_TAG, "Acta de traslado centro sanitario impresa correctamente")
                withContext(Dispatchers.Main) { onFinished() }
            } catch (e: Exception) {
                android.util.Log.e(BATCH_LOG_TAG, "Error al imprimir acta traslado centro sanitario: ${e.message}", e)
                withContext(Dispatchers.Main) { onError(friendlyErrorMessage(e)) }
            } finally {
                sharedConn?.let {
                    android.util.Log.i(BATCH_LOG_TAG, "Espera final antes de cerrar BT: ${FINAL_DRAIN_BEFORE_CLOSE_MS} ms")
                ...
                android.util.Log.i(BATCH_LOG_TAG, "Conexión BT de acta traslado centro sanitario cerrada")
            }
        }
    }

    // ============================================================
    // buildCentroSanitarioBody
    // Construye el cuerpo del documento de traslado a centro sanitario.
    // ============================================================
    private fun buildCentroSanitarioBody(
        doc: JSONObject,
        ph: Map<String, String>
    ): String {
        val sb = StringBuilder()
        ...
        doc.optJSONObject("lugar_intervencion")?.let { sec ->
            line(sec.optString("descripcion", ""))
            sec.optJSONArray("items")?.let { items ->
                for (i in 0 until items.length()) {
                    val item = items.getJSONObject(i)
                    line("  ${item.optString("label", "")} ${r(item.optString("campo", ""))}")
                }
            }
            blank()
        }
        ...
        doc.optJSONObject("persona")?.let { sec ->
            line(sec.optString("descripcion", ""))
            sec.optJSONArray("items")?.let { items ->
                for (i in 0 until items.length()) {
                    val item = items.getJSONObject(i)
        ...
        // 4. FACULTATIVO
        doc.optJSONObject("facultativo")?.let { sec ->
            line(sec.optString("descripcion", ""))
            sec.optJSONArray("items")?.let { items ->
                for (i in 0 until items.length()) {
                    val item = items.getJSONObject(i)
        ...
        // 5. PRUEBAS
        doc.optJSONObject("pruebas")?.let { sec ->
            line(sec.optString("descripcion", ""))
            sec.optJSONArray("items")?.let { items ->
                for (i in 0 until items.length()) {
                    val item = items.getJSONObject(i)
                    line("  [${r(item.optString("campo", ""))}] ${item.optString("label", "")}")
                }
            }
            blank()
        }
        ...
        // 6. CENTRO SANITARIO
        doc.optJSONObject("centro")?.let { sec ->
            line(sec.optString("descripcion", ""))
            sec.optJSONArray("items")?.let { items ->
                for (i in 0 until items.length()) {
                    val item = items.getJSONObject(i)
        ...
        // 7. MUESTRAS
        doc.optJSONObject("muestras")?.let { sec ->
            line(sec.optString("descripcion", ""))
            sec.optJSONArray("items")?.let { items ->
                for (i in 0 until items.length()) {
                    val item = items.getJSONObject(i)
        ...
        // 8. JUZGADO
        doc.optJSONObject("juzgado")?.let { sec ->
            line(sec.optString("descripcion", ""))
            sec.optJSONArray("items")?.let { items ->
                for (i in 0 until items.length()) {
                    val item = items.getJSONObject(i)
        ...
        doc.optJSONObject("otras_observaciones")?.let { sec ->
            line(sec.optString("descripcion", ""))
            sec.optJSONArray("items")?.let { items ->
                for (i in 0 until items.length()) {
                    val item = items.getJSONObject(i)
                    line("  ${r(item.optString("campo", ""))}")
                }
            }
            blank()
        }
        ...
        // 10. CIERRE
        doc.optJSONObject("cierre")?.let { sec ->
            line(sec.optString("descripcion", ""))
            sec.optJSONArray("items")?.let { items ->
                for (i in 0 until items.length()) {
                    val item = items.getJSONObject(i)
    ...
    // ============================================================
    // buildTrasladoBody
    // Construye el cuerpo del documento de traslado desde el JSON.
    // ============================================================
    private fun buildTrasladoBody(
        doc: JSONObject,
        ph: Map<String, String>
    ): String {
        val sb = StringBuilder()
        ...
        // 2. SECCIÓN ACTUACIÓN → marcador (se renderiza como caja CPCL)
        doc.optJSONObject("seccion_actuacion")?.let {
            sb.appendLine(TRASLADO_ACTUACION_MARKER)
        }

        // 3. CIRCUNSTANCIAS → marcador (se renderiza como caja CPCL)
        doc.optJSONObject("circunstancias")?.let {
            sb.appendLine(TRASLADO_CIRCUNSTANCIAS_MARKER)
        }

        // 4. INFORMACIÓN AL USUARIO
        doc.optJSONObject("informacion_traslado")?.let { sec ->
            line(sec.optString("descripcion", ""))
            sec.optJSONArray("parrafos")?.let { parrafos ->
                for (i in 0 until parrafos.length()) {
                    line("  ${parrafos.optString(i, "")}")
                }
            }
            blank()
        }

        // 5. INICIO / FIN DEL TRASLADO → marcador (se renderiza como tabla CPCL)
        doc.optJSONObject("inicio_fin_traslado")?.let {
            sb.appendLine(TRASLADO_INICIO_FIN_MARKER)
        }

        // 6. CONSENTIMIENTO Y AUTORIZACIÓN
        doc.optJSONObject("consentimiento")?.let { sec ->
            line(sec.optString("descripcion", ""))
            sec.optJSONArray("items")?.let { items ->
                for (i in 0 until items.length()) {
                    val item = items.getJSONObject(i)
        ...
        when {
            doc.has("cierre") && doc.get("cierre") is String -> {
                val cierre = doc.getString("cierre")
                if (cierre.isNotBlank()) line(r(cierre))
            }
        }
