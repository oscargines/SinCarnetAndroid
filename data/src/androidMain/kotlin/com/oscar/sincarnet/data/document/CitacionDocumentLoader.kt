...
data class CitacionDocument(
    ...
    val onlyInvestigatedSignature: Boolean = false,
    val omitInvestigatedSignature: Boolean = false,
...
internal fun replaceCitacionPlaceholders(
    ...
    result = result.replace("[[numeroexpediente]]", inicioModalData.numeroBoletin, ignoreCase = true)
    result = result.replace(
        "[[organismodestino]]",
        inicioModalData.jefaturaProvincial.takeIf { it.isNotBlank() }?.let {
            "Jefatura Provincial de Tráfico de $it"
        }.orEmpty(),
        ignoreCase = true
    )
    result = result.replace("[[juzgadodestino]]", courtData.sedeNombre, ignoreCase = true)
    result = result.replace(
        "[[antecedentesbdgc]]",
        if (inicioModalData.tieneAntecedentes && inicioModalData.antecedentesGuardiaCivil) "LOS INDICADOS EN ANEXO" else "NO CONSTA",
        ignoreCase = true
    )
    result = result.replace(
        "[[antecedentesbdsn]]",
        if (inicioModalData.tieneAntecedentes && inicioModalData.antecedentesSenalamientosNacionales) "LOS INDICADOS EN ANEXO" else "NO CONSTA",
        ignoreCase = true
    )
    result = result.replace("[[antecedentessis]]", "NO CONSTA", ignoreCase = true)
    result = result.replace(
        "[[antecedentesdgt]]",
        if (inicioModalData.tieneAntecedentes && inicioModalData.antecedentesDgt) "LOS INDICADOS EN ANEXO" else "NO CONSTAN",
        ignoreCase = true
    )
    result = result.replace(
        "[[antecedentesotroscuerpos]]",
        if (inicioModalData.tieneAntecedentes && inicioModalData.antecedentesOtrosCuerpos) "LOS INDICADOS EN ANEXO" else "NO CONSTAN",
        ignoreCase = true
    )
    result = result.replace(
        "[[requisitoriasjudiciales]]",
        if (inicioModalData.tieneAntecedentes && inicioModalData.requisitoriasJudiciales) "LOS INDICADOS EN ANEXO" else "NO CONSTAN",
        ignoreCase = true
    )
    val sistemaEnvio = if (inicioModalData.enviarPorLexnet) "LEXNET" else inicioModalData.modoEnvio
    result = result.replace("[[sistemaenvio]]", sistemaEnvio, ignoreCase = true)
    result = result.replace("[[horaenvio]]", LocalTime.now().format(DateTimeFormatter.ofPattern("HH:mm")), ignoreCase = true)
    result = result.replace("[[fechaenvio]]", LocalDate.now().format(DateTimeFormatter.ofPattern("dd-MM-yyyy")), ignoreCase = true)
    result = result.replace("[[numerofolios]]", "los que resulten", ignoreCase = true)
