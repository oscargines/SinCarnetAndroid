 ...
 * @param onGenerateCompleteAtestadoClick Abre la generación del atestado completo
...
fun TomaDatosAtestadoScreen(
    ...
    onGenerateCompleteAtestadoClick: () -> Unit = {},
    documentScanStatus: DocumentScanStatus = DocumentScanStatus.NO_COMPROBADO,
    ...
    var showDocumentRequirementDialog by remember { mutableStateOf(false) }

    if (showDocumentRequirementDialog) {
        AlertDialog(
            onDismissRequest = { showDocumentRequirementDialog = false },
            title = { Text(stringResource(R.string.atestado_complete_documents_required_title)) },
            text = {
                Text(
                    when (documentScanStatus) {
                        DocumentScanStatus.NO_COMPROBADO -> stringResource(
                            R.string.atestado_complete_documents_not_checked
                        )
                        DocumentScanStatus.DOCUMENTOS_INCOMPLETOS -> stringResource(
                            R.string.atestado_complete_documents_incomplete
                        )
                        DocumentScanStatus.SIN_DOCUMENTOS,
            ...
            confirmButton = {
                Button(onClick = {
                    showDocumentRequirementDialog = false
                    onScanDocumentClick()
                }) {
                    Text(stringResource(R.string.atestado_complete_documents_go_to_scan))
                }
            }
        )
    }
    ...
    Column(
        ...
        Card(
            ...
            Column(
                ...
                AtestadoActionButton(
                    text = stringResource(R.string.atestado_generate_complete),
                    onClick = {
                        if (documentScanStatus == DocumentScanStatus.SIN_DOCUMENTOS ||
                            documentScanStatus == DocumentScanStatus.DOCUMENTOS_COMPLETOS
                        ) {
                            onGenerateCompleteAtestadoClick()
                        } else {
                            showDocumentRequirementDialog = true
                        }
                    },
