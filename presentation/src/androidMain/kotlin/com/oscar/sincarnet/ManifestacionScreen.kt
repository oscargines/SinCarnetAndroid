...
import com.oscar.sincarnet.data.document.loadManifestacionDocument
...
fun ManifestacionScreen(
    ...
    val storage = remember(context) { ManifestacionStorage(context.toStorage("manifestacion_storage")) }
    val initialData = remember(context) { storage.loadCurrent() }
    val personData = remember(context) { PersonaInvestigadaStorage(context.toStorage("persona_investigada_storage")).loadCurrent() }
    val courtData = remember(context) { JuzgadoAtestadoStorage(context.toStorage("juzgado_atestado_storage")).loadCurrent() }
    val ocurrenciaData = remember(context) { OcurrenciaDelitStorage(context.toStorage("ocurrencia_delit_storage")).loadCurrent() }
    val vehicleData = remember(context) { VehiculoStorage(context.toStorage("vehiculo_storage")).loadCurrent() }
